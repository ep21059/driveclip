...
#SBATCH --output=/home/ryoc1220/DriveCLIP/logs/driveclip_infer.%A_%a.out
#SBATCH --error=/home/ryoc1220/DriveCLIP/logs/driveclip_infer.%A_%a.err
...
SIF_PATH=/home/ryoc1220/DriveCLIP/singularity/driveclip.sif
...
if torch.cuda.is_available():
    try:
        ...
        print('nvidia query failed:', e)
PY

  # 作業ディレクトリを repo に移動
  cd /workspace/${REPO_SUBDIR}

  # 出力先ディレクトリ（ホスト側のパスを使う）
  OUT_DIR=/workspace/data/frame_predictions/${VIDEO_ID}
  mkdir -p \"\${OUT_DIR}\"

  echo '=== Running inference script ==='
  python3 /workspace/${REPO_SUBDIR}/inference.py \
      --video /workspace/${VIDEO_REL} \
      --frame "\${OUT_DIR}" \
      --model_path /workspace/model_ckpt/syndd1_vitl14-per1_hypc_429_1000_ft.pkl

  echo '=== Copy / move results if necessary ==='
  # inference.py は OUT_DIR に frame_predictions_<video-stem>.json を書く想定
  ls -lh \${OUT_DIR} || true

  echo '=== inside container: end ==='
"

echo "[$(date)] JOB END: ${VIDEO_ID}"
